        ...
        if (currentGame == null) {
            ...
            switch (diff) {
                case KoteGame.HARD_DIFFICULTY:
                    ...
                    musicFilesIds = HARD_FILES_IDS;
                    break;

                case KoteGame.EXTREME_DIFFICULTY:
                    stringArrId = R.array.extreme_musical_parts;
                    musicFilesIds = EXTREME_FILES_IDS;
                    break;

                default:
                    stringArrId = R.array.easy_musical_parts;
                    musicFilesIds = EASY_FILES_IDS;
            }

            String[] stringArr = getApplication().getResources().getStringArray(stringArrId);
            ArrayList<Pair<String, Integer>> musicPairs = new ArrayList<>(10);
            for (int i = 0; i < stringArr.length && i < musicFilesIds.length; i++) {
                musicPairs.add(new Pair<>(stringArr[i], musicFilesIds[i]));
            }
            currentGame = new KoteGame(diff, musicPairs);
        }
    }
